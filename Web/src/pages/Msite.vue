<template>
  <div class="page">
   ...
   <mt-loadmore style="position:relative;" class="loadmores" :class="{'fixed-container':fixedHeader,'iphonex-tab-container':isIphoneX,'iphonex-loadmores':isIphoneX}" :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded"  ref="loadmore"  @top-status-change="handleTopChange" @bottom-status-change="handleBottomChange">
    ...
    <div slot="top" class="mint-loadmore-top">
      <div class="top_tip">
        ...
        <div v-show="topStatus !== 'loading'" class="reush_tip"><span class="txt">{{rank}}</span></div>
      </div>
    </div>
    ...
    <div slot="bottom" class="mint-loadmore-bottom" v-show="isShowDownLoad && !isNoData && !isNomle_bottom">
      <img v-show="bottomStatus !== 'loading'" src=".././assets/images/rush_icon.png"/>
        ...
        <div v-show="bottomStatus === 'loading'" class="icon"><mt-spinner type="fading-circle" color='#4E97FF'></mt-spinner></div>
...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapGetters(['userid','isIphoneX','deviceinfo','orgCodeList','orgCodeName']),
    },
    components: {
      Loadmore,
      pageSwiper,
    ...
    methods:{
      ...
      getGongList(){
        ...
        this.$ajax({method: 'get',url: '/mc-message/message/notice/list'+'/'+token,dataType: 'json', headers: {'Content-Type': 'application/json;charset=UTF-8'},
          data:JSON.stringify({
            token: this.userid,
          }),
        }).then(res=>{  // 请求成功
      ...
      getDingyueData(){
        ...
        this.$ajax(
          ...
          if(res.data.code==200){
            ...
            var newlist=[],swiperDatas=[],monthDatas={},showMD=false

            for(var i=0;i<this.situations.length;i++){
              if(this.situations[i].displayMode=="11"){//司南的今日概况
                swiperDatas.push(this.situations[i]);
                showMD=false
              }else if(this.situations[i].displayMode=="14"){//门店的今日概况
                swiperDatas.push(this.situations[i]);
                showMD=true
              }
              else if(this.situations[i].displayMode=="15"){//门店的本月概况
            ...
            this.$nextTick(()=>{
              ...
              if(this.swiperDataList.length<=0 && this.dataLists.length<=0 && !this.monthSituationList.title){
                this.isNoData=true
              }else{
                this.isNoData=false
              }
