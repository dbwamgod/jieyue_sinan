    ...
    methods:{
      ...
      createChartBox(){
        ...
        if(this.chartDetailData.pieLegend){ //如果是环形或者圆形, 无多种类型图表组合图的情况
          ...
          seriesItems.data.forEach((element,index) => {
            legendData.push({
              ...
              label:{//单独设置标签和标线是否显示，数值为0时不显示
                show: element.value==0 ? false :true,
              },
              labelLine: {
                show: element.value==0 ? false :true
              }
          ...
          this.isCircle = seriesItems.radius&&seriesItems.radius.length>0 ? true : false //是环形图的话，中间会显示文字
          ...
          for(let i=0; i<this.chartDetailData.series.length; i++){
            ...
            let seriesSettingsOption = {
              ...
              label: {
                normal: {
                  ...
                  show: seriesItem.valueShow==0?false:true, //节点是否显示数值
            ...
            if(seriesItem.type==='bar'){
             ...
             // 初始化colorlist变量为[]
              ...
              //console.log(seriesSettingsOption.data)
              // seriesSettingsOption.data=[
              // {}]
            } else {
              seriesSettingsOption.data = seriesItem.data;
            }
            //如果是柱形图e
            //如果是面积图s
            ...
            if(seriesItem.type=='line'){
              ...
              seriesSettingsOption.showSymbol=(seriesItem.symbolShow==0 ? false : true);//是否显示折线图的小圆点
              // 折线图节点圆点设置e
            }
            //如果是折线图e
